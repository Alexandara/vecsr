...
class MockVirtualHomeSimulator(Simulator):
	...
	def take_action(self, action):
		...
		if action[0].lower() == "walk":
			...
			# Check Preconditions
			relationships = self.check_for_edge(from_id=0, to_id=param1.identifier)
			if relationships:
				for rel in relationships:
					if rel["relation_type"] in ["INSIDE", "CLOSE", "HOLDS_RH", "HOLDS_LH"]:
						...
						return False
			if self.check_for_edge(from_id=0, relation_type="SITTING"):
				logging.warning("Character 0 currently sitting.")
				return False
			# Execute Action
			# Remove all close edges for character and held objects
			close_edges = self.check_for_edge(from_id=0, relation_type="CLOSE") + \
			              self.check_for_edge(to_id=0, relation_type="CLOSE")
			for close in close_edges:
				self.remove_edge(close)
			held_edges = self.check_for_edge(from_id=0, relation_type="HOLDS_RH") + \
			             self.check_for_edge(from_id=0, relation_type="HOLDS_LH")
			...
			for held in held_edges:
				...
				held_close_edges = self.check_for_edge(from_id=held["to_id"], relation_type="CLOSE") + \
				                   self.check_for_edge(to_id=held["to_id"], relation_type="CLOSE")
			...
			if param1.name in self.possible_rooms:
				...
				for item in moved_ids:
					if self.check_for_edge(from_id=item, relation_type="INSIDE"):
						character_inside_edges = self.check_for_edge(from_id=item, relation_type="INSIDE")
				...
				close_edges = self.check_for_edge(from_id=param1.identifier, relation_type="CLOSE") + \
				              self.check_for_edge(to_id=param1.identifier, relation_type="CLOSE")
				for item in moved_ids:
			...
			if not (self.check_for_edge(from_id=0, to_id=param1.identifier, relation_type="CLOSE") \
				...
				return False
			if "GRABBABLE" not in self.check_for_node(identifier=param1.identifier)[0]["properties"]:
				logging.warning("Item %s is not grabbable.", param1.to_string())
				return False
			if self.check_for_edge(from_id=0, relation_type="HOLDS_RH") \
				and self.check_for_edge(from_id=0, relation_type="HOLDS_LH"):
				logging.warning("Character 0 is holding something in both hands already "
				                "and cannot pick up another item.")
				return False
			for edge in self.check_for_edge(from_id=param1.identifier, relation_type="INSIDE"):
				# Note: this only checks if the item is inside one item, not if it's inside two or more objects
				if "CLOSED" in self.check_for_node(identifier=edge["to_id"])[0]["states"]:
			...
			for edge in self.check_for_edge(from_id=param1.identifier, relation_type="ON") \
			            + self.check_for_edge(from_id=param1.identifier, relation_type="INSIDE"):
			...
			for edge in self.check_for_edge(from_id=0, relation_type="CLOSE"):
				if param1.identifier != edge["to_id"]:
					self.state_graph["edges"].append({"from_id":param1.identifier, "to_id":edge["to_id"],
					                                  "relation_type":"CLOSE"})
			for edge in self.check_for_edge(to_id=0, relation_type="CLOSE"):
	...
	def check_for_nodes_of_state_or_property(self, s_or_p):
	...
	def check_for_edge(self, from_id=None, to_id=None, relation_type=None):
